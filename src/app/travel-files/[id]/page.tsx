  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="border-primary/20 shadow-md">
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <InfoRow label="Departure" value={file.departure_date ? formatDateOnly(file.departure_date) : "—"} />
              <InfoRow label="Return" value={file.return_date ? formatDateOnly(file.return_date) : "—"} />
              <InfoRow label="Assigned Advisor" value={file.assigned_advisor?.full_name ?? "Unassigned"} />
            ...
            {currentAction ? (
              <div className="space-y-3">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <InfoRow label="Responsible" value={getResponsibleName(currentAction, profileMap)} />
                  <InfoRow
                    label="Due / Waiting"
                    value={
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <InfoRow label="Agreement Type" value={file.tmf_agreement_type === "ivt" ? "IVT" : "All-Inclusive"} />
                <InfoRow label="TMF Amount" value={formatCurrency(file.tmf_amount)} />
                <InfoRow label="Assigned Advisor" value={file.assigned_advisor?.full_name ?? "Unassigned"} />
                {file.tmf_agreement_type === "ivt" && (
                  <InfoRow label="Revisions Included" value={file.revisions_included != null ? String(file.revisions_included) : "—"} />
                )}
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <InfoRow label="Inquiry Received" value={formatReadableDate(file.inquiry_received_at)} />
              <InfoRow label="Destination" value={file.destination} />
              <InfoRow label="Trip Type" value={file.trip_type} />
              <InfoRow label="Number of Travellers" value={file.number_of_travellers} />
              <InfoRow label="Departure Date" value={file.departure_date ? formatDateOnly(file.departure_date) : "—"} />
              <InfoRow label="Return Date" value={file.return_date ? formatDateOnly(file.return_date) : "—"} />
              <InfoRow label="Budget Range" value={file.budget_range} />
              <InfoRow label="TMF Amount" value={formatCurrency(file.tmf_amount)} />
              <InfoRow label="IVT / Custom" value={formatBoolean(file.ivt_custom)} />
              <InfoRow label="Proposal Due Date" value={file.proposal_due_date ? formatDateOnly(file.proposal_due_date) : "—"} />
              <InfoRow label="Date Booked" value={file.date_booked ? formatDateOnly(file.date_booked) : "—"} />
              <InfoRow label="Total Booking Value" value={formatCurrency(file.total_booking_value)} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><CardTitle>Booking References</CardTitle></CardHeader>
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                {file.travefy_proposal_url ? (
                  <a href={file.travefy_proposal_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline break-all">
                    {file.travefy_proposal_url}
                  </a>
              ...
              <div className="flex flex-col gap-0.5">
                ...
                {file.travefy_trip_plan_url ? (
                  <a href={file.travefy_trip_plan_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline break-all">
                    {file.travefy_trip_plan_url}
                  </a>
                ) : <span className="text-sm font-medium text-foreground">—</span>}
              </div>
        ...
        <Card>
          <CardHeader><CardTitle>Insurance / Pre-Trip</CardTitle></CardHeader>
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <InfoRow label="Pre-Trip Meeting Required" value={formatBoolean(file.pretrip_meeting_required)} />
              <InfoRow label="Pre-Trip Meeting Booked" value={file.pretrip_meeting_booked_at ? formatReadableDateTime(file.pretrip_meeting_booked_at) : "—"} />
              <InfoRow label="Pre-Trip Card Sent" value={file.pretrip_card_sent_at ? formatReadableDateTime(file.pretrip_card_sent_at) : "—"} />
              <InfoRow label="Booking Registration Eligible" value={formatBoolean(file.booking_registration_eligible)} />
              <InfoRow label="Booking Registration Completed" value={file.booking_registration_done_at ? formatReadableDateTime(file.booking_registration_done_at) : "—"} />
            </div>
          </CardContent>
        </Card>
                  ...
                  <div key={payment.id} className="py-3 flex flex-wrap items-center justify-between gap-2">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        {payment.description && <span className="text-sm text-muted-foreground">{payment.description}</span>}
                      </div>
                      <span className="text-xs text-muted-foreground">Due: {formatDateOnly(payment.due_date)}</span>
                    </div>
              ...
              <div className="divide-y divide-border">
                {consultations.map((c) => (
                  <div key={c.id} className="py-3 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-medium text-foreground">{formatReadableDate(c.consulted_at)}</span>
                      <Badge variant="secondary" className="capitalize">{c.outcome.replace(/_/g, " ")}</Badge>
                      <span className="text-xs text-muted-foreground">by {c.conducted_by_profile?.full_name ?? "Unknown"}</span>
                    </div>
                    {c.discussion_summary && <p className="text-sm text-muted-foreground">{c.discussion_summary}</p>}
                    {c.recommendations && (
                      <p className="text-xs text-muted-foreground"><span className="font-medium">Recommendations:</span> {c.recommendations}</p>
                    )}
                    {c.next_steps && (
                      <p className="text-xs text-muted-foreground"><span className="font-medium">Next steps:</span> {c.next_steps}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <Card>
          ...
          <CardContent>
            {activity.length === 0 ? (
              ...
              <div className="divide-y divide-border">
                {visibleActivity.map((a) => <ActivityHistoryItem key={a.id} activity={a} />)}
                {additionalActivity.length > 0 && (
                  <details className="group border-t border-border">
                    ...
                    <div className="divide-y divide-border border-t border-border">
                      {additionalActivity.map((a) => <ActivityHistoryItem key={a.id} activity={a} />)}
                    </div>
                  </details>
                )}
              </div>
