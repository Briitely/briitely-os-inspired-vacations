...
import {addCalendarDaysRollingWeekend} from "@/lib/travel/due-date";
...
export async function GET(_request:Request,{params}:{params:Promise<{travelFileId:string}>}){
 ...
 const{travelFileId}=await params;const db=await createClient();const{data:file,error}=await db.from("travel_files").select("current_action:travel_actions!current_action_id(action_code,status,metadata)").eq("id",travelFileId).maybeSingle();if(error||!file)return NextResponse.json({error:"Travel File not found."},{status:404});const current=Array.isArray(file.current_action)?file.current_action[0]:file.current_action;if(!current||current.action_code!=="check_proposal_status"||current.status!=="active")return NextResponse.json({error:"Check Proposal Status is not the active action."},{status:409});return NextResponse.json({followUpNumber:Number((current.metadata as Record<string,unknown>|null)?.follow_up_number??1)});
}
...
export async function POST(request:Request,{params}:{params:Promise<{travelFileId:string}>}){
 ...
 if(body.outcome==="no_activity"){
  const nextFollowUp=followUp+1;const dueDays=followUp===1?2:5;const dueAt=addCalendarDaysRollingWeekend(dueDays,new Date(now));const title=followUp===1?"Check Proposal Status — Second Follow-up":"Review Opportunity Status";const description=followUp===1?"Send the first proposal reminder, then check again for a client view or response.":"No client activity after proposal follow-up. Review whether to mark the proposal inactive.";
  const{data:next,error:nextError}=await db.from("travel_actions").insert({travel_file_id:travelFileId,action_code:followUp===1?"check_proposal_status":"review_proposal_opportunity",title,description,action_role:"blocking",responsible_type:"internal",responsible_user_id:file.assigned_advisor_id,status:"active",due_at:dueAt,waiting_since:now,activated_at:now,metadata:{follow_up_number:nextFollowUp,reminder_required:true}}).select("id").single();if(nextError||!next)return NextResponse.json({error:"Could not create the next proposal follow-up."},{status:500});
  ...
  await db.from("travel_activity").insert({travel_file_id:travelFileId,event_type:"proposal_follow_up",summary:followUp===1?"No proposal activity; first reminder is due and another check is scheduled in 2 days (rolled to Monday when needed).":"No proposal activity after follow-up; opportunity review scheduled, with weekend due dates rolled to Monday.",actor_type:"internal",actor_user_id:user.id,action_id:current.id,metadata:{follow_up_number:nextFollowUp,due_at:dueAt}});return NextResponse.json({success:true,nextAction:title});
 }
