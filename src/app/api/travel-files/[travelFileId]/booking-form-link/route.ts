...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ url: `${origin}/booking/${encodeURIComponent(token)}`, expiresAt, includeRetainer: false });
  } catch (error) {
    console.error("BOOKING_FORM_LINK_CREATE_FAILED", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not create booking form link." }, { status: 500 });
  }
}
