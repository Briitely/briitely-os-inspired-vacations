...
export async function GET(_req: Request, { params }: { params: Promise<{ travelFileId: string }> }) {
  ...
  try {
    const { data: openTask, error: taskError } = await s.from("travel_file_tasks").select("id").eq("travel_file_id", travelFileId).eq("title", DUPLICATE_REVIEW_TITLE).neq("status", "complete").order("created_at", { ascending: false }).limit(1).maybeSingle();
    if (taskError) throw new Error(taskError.message);
    if (!openTask) return NextResponse.json({ travellers: [] });

    const { data: submissions, error: submissionError } = await s.from("booking_form_submissions").select("booking_form_session_id,submitted_at").eq("travel_file_id", travelFileId).order("submitted_at", { ascending: false });
    ...
    for (const submission of submissions) if (submission.booking_form_session_id && !submissionBySession.has(submission.booking_form_session_id)) submissionBySession.set(submission.booking_form_session_id, submission.submitted_at);

    const highlighted = new Map<string, { id: string; name: string }>();
    for (const session of sessions ?? []) {
      const submittedAt = submissionBySession.get(session.id);
      if (!submittedAt) continue;
      const sessionStart = new Date(session.created_at).getTime(), sessionEnd = new Date(submittedAt).getTime();
      ...
      for (const member of members ?? []) {
        ...
        const sameFormGroup = session.recipient_party_member_id ? member.id === session.recipient_party_member_id || member.booking_form_recipient_party_member_id === session.recipient_party_member_id : !member.booking_form_recipient_party_member_id;
        if (!sameFormGroup) continue;
        const profile = profileOf(member), name = [profile?.preferred_name || profile?.first_name, profile?.last_name].filter(Boolean).join(" ");
