...
export async function POST(request: Request, { params }: { params: Promise<{ travelFileId: string }> }) {
  ...
  try {
    await setContactCustomFieldByKey({
      contactId: f.briitely_contact_id,
      fieldKey: BOOKING_FORM_LINK_FIELD_KEY,
      value: bookingUrl,
    });
    await sendNamedEmailTemplate({
      contactId: f.briitely_contact_id,
      templateName: RETAINER_EMAIL_TEMPLATE,
    });
  } catch (error) {
    console.error("RETAINER_EMAIL_SEND_FAILED", error);
    await supabase.from("booking_form_sessions").delete().eq("id", session.id);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not send the Retainer Agreement email through Briitely." }, { status: 502 });
  }

  const now = new Date().toISOString();
  const { error: completeError } = await supabase
    .from("travel_actions")
    .update({ status: "completed", completed_at: now, completed_by: user.id, completion_source: "portal" })
    .eq("id", a.id);
  ...
  const { data: newAction, error: createError } = await supabase
    ...
    .insert({
      travel_file_id: travelFileId,
      action_code: "await_tmf_and_booking_form",
      title: "Await Retainer Agreement & Booking Form",
      action_role: "blocking",
      responsible_type: "client",
      status: "active",
      waiting_since: now,
      activated_at: now,
      metadata: { delivery: "portal_booking_link" },
    })
    .select("id")
    .single();
  ...
  const { error: updateError } = await supabase
    .from("travel_files")
    .update({ stage: "tmf_sent", stage_changed_at: now, current_action_id: newAction.id, tmf_sent_at: now, tmf_document_id: null })
    .eq("id", travelFileId);
  ...
  await supabase.from("travel_activity").insert({
    ...
    summary: "Retainer Agreement and Booking Form link emailed to client.",
    actor_type: "internal",
    ...
    new_stage: "tmf_sent",
    metadata: {
      delivery: "briitely_email_template",
      template: RETAINER_EMAIL_TEMPLATE,
      bookingForm: true,
      bookingFormLinkFieldKey: BOOKING_FORM_LINK_FIELD_KEY,
    },
  });
  ...
  return NextResponse.json({
    ...
    bookingUrl,
    expiresAt,
    nextAction: { id: newAction.id, title: "Await Retainer Agreement & Booking Form", actionCode: "await_tmf_and_booking_form" },
  });
}
