      ...
      <main className="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground"><CreditCard className="h-6 w-6" /></div>
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Receive Payment</h1>
              <p className="mt-2 text-base text-muted-foreground">Record a cheque or e-transfer against an unpaid invoice.</p>
            </div>
          </div>
        </div>
        <PaymentFlow initialCustomer={parseJsonParam(params.customer)} initialInvoice={parseJsonParam(params.invoice)} />
      </main>
