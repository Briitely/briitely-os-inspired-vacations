...
export function SendTmfModal({
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? "Failed to send TMF Agreement.");
        setSending(false);
        return;
      }

      if (data.result === "already_sent") {
        setError("This TMF Agreement has already been sent.");
        setSending(false);
        return;
      }
        ...
        <CardContent className="space-y-5 overflow-y-auto flex-1 p-6">
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-900 dark:bg-blue-950/30">
            <p className="text-sm text-blue-800 dark:text-blue-200">
              Review the agreement details below. If anything is incorrect, close this dialog
              and edit the Travel File first. Clicking Send will deliver the agreement to the
              client via Briitely.
            </p>
          </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {isIvt && (
                <ReviewField
                  ...
                  value={revisionsIncluded != null ? String(revisionsIncluded) : "—"}
                />
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              The correct Briitely template will be selected automatically based on the agreement type.
            </span>
          </div>
