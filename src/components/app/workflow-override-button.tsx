...
export function WorkflowOverrideButton({
  ...
  useEffect(() => {
    let active = true;
    fetch(`/api/travel-files/${encodeURIComponent(travelFileId)}/current-action`)
      .then((response) => (response.ok ? response.json() : null))
      .then((data) => {
        if (!active || !data?.action) return;
        setDueAt(data.action.due_at ?? null);
        setNotes(data.action.notes ?? null);
      })
      .catch(() => {});
    return () => {
      active = false;
  ...
  return (
    <>
      <div className="workflow-action-controls w-full">
        ...
        {currentActionCode && (
          <CurrentActionControls travelFileId={travelFileId} dueAt={dueAt} notes={notes} />
        )}
      </div>
