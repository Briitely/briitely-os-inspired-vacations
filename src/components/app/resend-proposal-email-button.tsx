...
export function ResendProposalEmailButton({travelFileId}:{travelFileId:string}){
 ...
 if(!available)return null;return <div className="space-y-1"><Button variant="outline" size="sm" className="w-full justify-start" onClick={resend} disabled={loading}>{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<Mail className="h-4 w-4"/>}Resend Proposal Email</Button>{error&&<p className="text-xs text-destructive">{error}</p>}</div>
}
