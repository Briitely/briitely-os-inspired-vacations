...
-- Match the portal's existing authenticated-staff model. API/server access continues
-- through the authenticated Supabase client and existing profile role checks.
...
-- Backfill a client-level profile from the most recently updated Travel File for
-- each contact so existing intake interests/seasons are not lost when the UI moves.
