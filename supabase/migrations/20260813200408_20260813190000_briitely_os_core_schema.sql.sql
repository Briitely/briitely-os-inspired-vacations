...
2. New Tables
   ...
   - `invoice_commissions`
     - (Already exists from prior migration; created with IF NOT EXISTS)
     - `id` uuid, primary key, default gen_random_uuid()
...
6. Security (RLS)
   ...
   - `invoice_commissions`: RLS enabled
     - All CRUD for authenticated users (existing policies preserved)
...
7. Seeded Defaults
   ...
   - `invoiceGoLiveDate`: NOT seeded — no cutoff until admin sets one
   - All seeded values are bootstrap fallbacks only. Admin-configured values
     in client_settings override these defaults. Application code falls back
     to config defaults when a database setting is absent.

8. Important Notes
   - This migration is fully idempotent (IF NOT EXISTS, CREATE OR REPLACE,
     DROP POLICY/TRIGGER IF EXISTS throughout) and safe to re-run.
   - No existing data is modified or deleted.
   - The `invoice_commissions` table may already exist from a prior migration;
     CREATE TABLE IF NOT EXISTS handles this gracefully.
   - The `app_role` enum includes all three values (super_admin, admin, staff)
     in a single creation, since this targets a fresh database.
*/
...
-- =========================================================
-- 6. invoice_commissions table (may already exist)
-- =========================================================
...
-- =========================================================
-- 10. Admin helper functions (must exist before RLS policies
--     that reference them)
-- =========================================================

-- is_admin(): returns true for active admin or super_admin
...
-- is_super_admin(): returns true only for active super_admin
...
-- ---- integration_log ----
-- Admin-only SELECT; no INSERT/UPDATE/DELETE for authenticated.
...
-- =========================================================
-- 13. admin_update_profile function (with role-cast fix)
-- =========================================================
...
--     These are generic, client-agnostic fallbacks only.
--     Admin-configured values override these via the same
--     upsert path. Application code falls back to config
--     defaults when a database row is absent.
-- =========================================================
